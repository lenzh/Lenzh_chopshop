...
local lastTested = 0
local result = false--detect if we need to trigger alert if vehicle isn't in table
function ChopVehicle()
	local ped = PlayerPedId()
	local _vehicle = GetVehiclePedIsIn(ped, false)
	local model = GetEntityModel(_vehicle)
	...
	for k,v in pairs(vehicleName) do --only chop if vehicle is in table.
		...
		if displayText == v then
			...
			GetGameTimer() - lastTested > Config.CooldownMinutes * 60000 then
				...
				table.remove(vehicleName,index) --remove the vehicle rom our table.
				VehiclePartsRemoval()
...
function VehiclePartsRemoval()
	...
	Citizen.Wait(1000)
	exports['progressBars']:startUI(Config.DeletingVehicleTime, "Scrapping Vehicle")
...
AddEventHandler('lenzh_chopshop:hasEnteredMarker', function(zone)
	if zone == 'Chopshop' and IsDriver() then
		...
		CurrentActionData = {}
	elseif zone == 'getOrders' then
		CurrentAction     = 'getOrders'
		CurrentActionMsg  = _U('get_orders')
		CurrentActionData = {}
	end
end)
	...
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlJustReleased(0, 38) then
				...
				elseif CurrentAction == 'getOrders' then
					ESX.TriggerServerCallback('lenzh_chopshop:getOrders', function(_table)
						for k,v in pairs(_table) do
							local displayText = GetDisplayNameFromVehicleModel(v)
							local vehicleModel = GetLabelText(displayText)
							table.insert(vehicleName,vehicleModel)
						end
					end, vehicleName)
				end
				CurrentAction = nil
