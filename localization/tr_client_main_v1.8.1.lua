...
function ChopVehicle()
    ...
    if seats ~= 0 then
        TriggerEvent('chat:addMessage', { args = { '[^1Chopshop^0]: Araçta yolcu varken parçalayamazsınız.' } })
    elseif
        ...
        ESX.TriggerServerCallback('Lenzh_chopshop:anycops', function(anycops)
            if anycops >= Config.CopsRequired then
                ...
                if not HasAlreadyEnteredMarker then
                    ...
                    exports.pNotify:SendNotification({text = "Bölgeyi terk ettiniz. Ödül alamayacaksınız.", type = "error", timeout = 1000, layout = "centerRight", queue = "right", killer = true, animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
        ...
        exports.pNotify:SendNotification({
            text = "Tekrar parçalama yapabilmek için " ..math.floor(timerNewChop / 60000).. " dakika(s)",
            type = "error",
            timeout = 1000,
            layout = "centerRight",
            queue = "right",
...
function VehiclePartsRemoval()
    ...
    if ChoppingInProgress == true then
        exports['progressBars']:startUI(Config.DoorOpenFrontLeftTime, "Sol ön kapı açılıyor")
        Citizen.Wait(Config.DoorOpenFrontLeftTime)
        SetVehicleDoorOpen(GetVehiclePedIsIn(ped, false), 0, false, false)
    end
    Citizen.Wait(1000)
    if ChoppingInProgress == true then
        exports['progressBars']:startUI(Config.DoorBrokenFrontLeftTime, "Sol ön kapı sökülüyor")
        Citizen.Wait(Config.DoorBrokenFrontLeftTime)
        SetVehicleDoorBroken(GetVehiclePedIsIn(ped, false), 0, true)
    end
    Citizen.Wait(1000)
    if ChoppingInProgress == true then
        exports['progressBars']:startUI(Config.DoorOpenFrontRightTime, "Sağ ön kapı açılıyor")
        Citizen.Wait(Config.DoorOpenFrontRightTime)
        SetVehicleDoorOpen(GetVehiclePedIsIn(ped, false), 1, false, false)
    end
    Citizen.Wait(1000)
    if ChoppingInProgress == true then
        exports['progressBars']:startUI(Config.DoorBrokenFrontRightTime, "Sağ ön kapı sökülüyor")
	...
	if rearLeftDoor ~= -1 then
        if ChoppingInProgress == true then
            exports['progressBars']:startUI(Config.DoorOpenRearLeftTime, "Sol arka kapı açılıyor")
            Citizen.Wait(Config.DoorOpenRearLeftTime)
            SetVehicleDoorOpen(GetVehiclePedIsIn(ped, false), 2, false, false)
        end
        Citizen.Wait(1000)
        if ChoppingInProgress == true then
            exports['progressBars']:startUI(Config.DoorBrokenRearLeftTime, "Sol arka kapı sökülüyor")
            Citizen.Wait(Config.DoorBrokenRearLeftTime)
            SetVehicleDoorBroken(GetVehiclePedIsIn(ped, false), 2, true)
        end
        Citizen.Wait(1000)
        if ChoppingInProgress == true then
            exports['progressBars']:startUI(Config.DoorOpenRearRightTime, "Sağ arka kapı açılıyor")
            Citizen.Wait(Config.DoorOpenRearRightTime)
            SetVehicleDoorOpen(GetVehiclePedIsIn(ped, false), 3, false, false)
        end
        Citizen.Wait(1000)
        if ChoppingInProgress == true then
            exports['progressBars']:startUI(Config.DoorBrokenRearRightTime, "Sağ arka kapı sökülüyor")
	...
	if bonnet ~= -1 then
        if ChoppingInProgress == true then
            exports['progressBars']:startUI(Config.DoorOpenHoodTime, "Kaput açılıyor")
            Citizen.Wait(Config.DoorOpenHoodTime)
            SetVehicleDoorOpen(GetVehiclePedIsIn(ped, false), 4, false, false)
        end
        Citizen.Wait(1000)
        if ChoppingInProgress == true then
            exports['progressBars']:startUI(Config.DoorBrokenHoodTime, "Kaput sökülüyor")
	...
	if boot ~= -1 then
        if ChoppingInProgress == true then
            exports['progressBars']:startUI(Config.DoorOpenTrunkTime, "Bagaj açılıyor")
            Citizen.Wait(Config.DoorOpenTrunkTime)
            SetVehicleDoorOpen(GetVehiclePedIsIn(ped, false), 5, false, false)
        end
        Citizen.Wait(1000)
        if ChoppingInProgress == true then
            exports['progressBars']:startUI(Config.DoorBrokenTrunkTime, "Bagaj sökülüyor")
            Citizen.Wait(Config.DoorBrokenTrunkTime)
            SetVehicleDoorBroken(GetVehiclePedIsIn(ped, false),5, true)
        end
    end
    Citizen.Wait(1000)
    exports['progressBars']:startUI(Config.DeletingVehicleTime, "Gerisini John'nun halletmesine izin ver!")
    Citizen.Wait(Config.DeletingVehicleTime)
    if ChoppingInProgress == true then
        DeleteVehicle()
        exports.pNotify:SendNotification({text = "Araç Başarıyla Parçalandı...", type = "success", timeout = 1000, layout = "centerRight", queue = "right", animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
    end
end
...
AddEventHandler('lenzh_chopshop:hasExitedMarker', function(zone)
    ...
    if zone == 'Chopshop' then
        ...
        if ChoppingInProgress == true then
            exports.pNotify:SendNotification({text = "Alanı terk ettiniz. Alana geri dönün.", type = "error", timeout = 1000, layout = "centerRight", queue = "right", killer = true, animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
        end
    end
    ChoppingInProgress        = false
